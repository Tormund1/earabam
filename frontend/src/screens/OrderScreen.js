...
export default function OrderScreen() {
  ...
  return loading ? (
    ...
    <div>
      <Helmet>
        <title>Siparis:{orderId}</title>
      </Helmet>
      <h1 className="my-3">Sipariş Numarası: {orderId}</h1>
      <Row>
        <Col md={8}>
          <Card className="mb-3">
            <Card.Body>
          ...
          <Card className="mb-3">
            <Card.Body>
              ...
              <Card.Text>
                <strong>Ödeme Metodu:</strong> {order.paymentMethod}
              </Card.Text>
              {order.isPaid ? (
                <MessageBox variant="success">Ödendi {order.paidAt}</MessageBox>
              ) : (
